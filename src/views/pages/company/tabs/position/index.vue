...
import { mapEmploymentType } from './employment-type.map';
import { inject } from 'vue';
import { mapPaymentType } from './payment-type.map';
...
watch(companyContext, async (value) => {
  positionStore.setCompany(companyContext.value.id)

  try
  {
    const { status: code, data: response } = await PositionService.getPositionByCompanyId(value.id)
